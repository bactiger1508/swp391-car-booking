            ...
            <table class="table">
                ...
                <tbody>
                    <c:forEach var="h" items="${handovers}">
                        <tr onclick="window.location.href = 'handovers/create?bookingId=${h.bookingId}&carId=${h.carId}'">
                            <td>${h.handoverId}</td><td>#${h.bookingId}</td><td>Car #${h.carId}</td><td>${h.handoverDate}</td><td>${h.mileageAtHandover} km</td><td>${h.fuelLevel}</td><td>Staff #${h.handedBy}</td>
                        </tr>
                    </c:forEach>
                </tbody>
            </table>
