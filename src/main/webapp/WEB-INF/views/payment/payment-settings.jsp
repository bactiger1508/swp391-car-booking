    ...
    <%@ taglib prefix="c" uri="jakarta.tags.core" %>
        ...
        <div class="px-8 py-6 max-w-[1400px] mx-auto bg-surface min-h-screen text-on-surface">
            ...
            <c:if test="${not empty successMsg}">
                <div
                    class="mb-6 p-4 bg-emerald-50 border-l-4 border-emerald-500 rounded-r-xl shadow-sm flex items-center justify-between transition-all duration-300">
                    <div class="flex items-center gap-3">
                        <span class="material-symbols-outlined text-emerald-600 font-bold"
                            style="font-variation-settings: 'FILL' 1">check_circle</span>
                        <span class="text-sm font-semibold text-emerald-800">${successMsg}</span>
                    </div>
                    <button onclick="this.parentElement.remove()"
                        class="text-emerald-500 hover:text-emerald-700 text-lg font-bold">&times;</button>
                </div>
            </c:if>
            <c:if test="${not empty errorMsg}">
                <div
                    class="mb-6 p-4 bg-red-50 border-l-4 border-red-500 rounded-r-xl shadow-sm flex items-center justify-between transition-all duration-300">
                    <div class="flex items-center gap-3">
                        <span class="material-symbols-outlined text-red-600 font-bold"
            ...
            <form method="POST" action="${pageContext.request.contextPath}/admin/payment-settings" class="space-y-6">
                ...
                <div class="mb-8 flex justify-between items-end">
                    <div>
                        <nav class="flex text-sm text-outline mb-2">
                            ...
                            <span class="text-primary font-medium">Cấu hình chính sách &amp; Phương thức thanh
                                toán</span>
                        </nav>
                        <h3 class="text-3xl font-bold tracking-tight text-on-surface">Thiết lập Quy tắc &amp; Thanh toán
                        </h3>
                        <p class="text-on-surface-variant mt-1">Điều chỉnh các thông số vận hành và quản lý luồng tiền
                            tệ của hệ thống.</p>
                    </div>
                    <div class="flex gap-3">
                        <a href="${pageContext.request.contextPath}/admin/payment-settings"
                            class="px-6 py-2.5 rounded-full border border-outline text-on-surface font-medium hover:bg-surface-container-high transition-all active:scale-95 text-sm inline-flex items-center justify-center">
                            Hủy thay đổi
                        </a>
                        <button type="submit"
                            class="px-6 py-2.5 rounded-full bg-primary text-on-primary font-medium shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all active:scale-95 text-sm flex items-center gap-2 cursor-pointer">
                ...
                <div class="grid grid-cols-12 gap-6">
                    ...
                    <div class="col-span-12 lg:col-span-8 space-y-6">
                        ...
                        <div class="glass-card rounded-3xl p-8 shadow-sm scroll-mt-20" id="bank-config-card">
                            <div class="flex items-center gap-3 mb-6">
                                <div
                                    class="w-12 h-12 rounded-2xl bg-amber-500/10 flex items-center justify-center text-amber-600">
                                    <span class="material-symbols-outlined"
                                        style="font-variation-settings: 'FILL' 1">account_balance</span>
                                </div>
                                <div>
                                    <h4 class="text-xl font-bold text-on-surface">Cấu hình Tài khoản Ngân hàng Doanh
                                        nghiệp</h4>
                                    <p class="text-sm text-on-surface-variant">Thông tin tài khoản nhận tiền chuyển
                                        khoản tự động và liên kết sinh mã VietQR.</p>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-6">
                                <!-- Bank Name -->
                                <c:set var="bankName"
                                    value="${settingMap['BANK_NAME'] != null ? settingMap['BANK_NAME'].policyValue : ''}" />
                                <label class="block">
                                    <span class="text-sm font-semibold text-on-surface mb-2 block">Tên Ngân Hàng Doanh
                                        Nghiệp *</span>
                                    <div
                                        class="relative rounded-xl overflow-hidden border border-outline-variant focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary transition-all">
                                        <input
                                            class="w-full bg-surface-container-low border-none px-4 py-3 outline-none transition-all font-medium text-on-surface"
                                            type="text" name="policy_BANK_NAME" value="${bankName}"
                                            placeholder="Techcombank, Vietcombank..." required />
                                    </div>
                                </label>

                                <!-- Bank Branch -->
                                <c:set var="bankBranch"
                                    value="${settingMap['BANK_BRANCH'] != null ? settingMap['BANK_BRANCH'].policyValue : ''}" />
                                <label class="block">
                                    <span class="text-sm font-semibold text-on-surface mb-2 block">Chi Nhánh Của Ngân
                                        Hàng *</span>
                                    <div
                                        class="relative rounded-xl overflow-hidden border border-outline-variant focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary transition-all">
                                        <input
                                            class="w-full bg-surface-container-low border-none px-4 py-3 outline-none transition-all font-medium text-on-surface"
                                            type="text" name="policy_BANK_BRANCH" value="${bankBranch}"
                                            placeholder="Chi nhánh Thạch Thất..." required />
                                    </div>
                                </label>

                                <!-- Account Number -->
                                <c:set var="accNumber"
                                    value="${settingMap['BANK_ACCOUNT_NUMBER'] != null ? settingMap['BANK_ACCOUNT_NUMBER'].policyValue : ''}" />
                                <label class="block">
                                    <span class="text-sm font-semibold text-on-surface mb-2 block">Số Tài Khoản Nhận
                                        *</span>
                                    <div
                                        class="relative rounded-xl overflow-hidden border border-outline-variant focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary transition-all">
                                        <input
                                            class="w-full bg-surface-container-low border-none px-4 py-3 outline-none transition-all font-medium text-on-surface"
                                            type="text" name="policy_BANK_ACCOUNT_NUMBER" value="${accNumber}"
                                ...
                                <!-- Account Name -->
                                <c:set var="accName"
                                    value="${settingMap['BANK_ACCOUNT_NAME'] != null ? settingMap['BANK_ACCOUNT_NAME'].policyValue : ''}" />
                                <label class="block">
                                    <span class="text-sm font-semibold text-on-surface mb-2 block">Tên Chủ Tài Khoản
                                        (Không dấu) *</span>
                                    <div
                                        class="relative rounded-xl overflow-hidden border border-outline-variant focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary transition-all">
                                        <input
                    ...
                    <div class="col-span-12 lg:col-span-4 space-y-6">
                        ...
                        <div
                            class="bg-white rounded-3xl p-8 shadow-xl border border-primary/5 relative overflow-hidden h-fit">
                            ...
                            <div class="relative z-10 space-y-6">
                                ...
                                <div class="flex items-center gap-3 mb-4">
                                    <div
                                        class="w-12 h-12 rounded-2xl bg-on-primary-container flex items-center justify-center text-primary-container shrink-0">
                                        <span class="material-symbols-outlined"
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-on-surface-variant">Bật/tắt các cổng thanh toán hệ thống.
                                        </p>
                                    </div>
                                </div>

                                <!-- 1. CASH method -->
                                <c:set var="cashEnabled"
                                    value="${methodMap['PAYMENT_METHOD_CASH_ENABLED'] != null && methodMap['PAYMENT_METHOD_CASH_ENABLED'].policyValue == 'true'}" />
                                <div
                                    class="group relative p-5 bg-surface-container-low rounded-2xl border-2 border-transparent hover:border-primary/20 transition-all ${cashEnabled ? '' : 'opacity-60'}">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center gap-4">
                                            <div
                                                class="w-10 h-10 rounded-lg bg-white flex items-center justify-center shadow-sm text-secondary font-bold">
                                                💵
                                            </div>
                                            <div>
                                                <p class="font-bold text-on-surface text-sm">Tiền mặt</p>
                                                <p class="text-[11px] text-on-surface-variant italic">Thanh toán trực
                                                    tiếp tại quầy</p>
                                            </div>
                                        </div>
                                        <label class="relative inline-flex items-center cursor-pointer">
                                            <input type="checkbox" name="PAYMENT_METHOD_CASH_ENABLED" value="on"
                                                ${cashEnabled ? 'checked' : '' } class="sr-only peer"
                                                onchange="toggleCardOpacity(this)">
                                            <div
                                                class="w-11 h-6 bg-outline-variant peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary">
                                            </div>
                                        </label>
                                ...
                                <c:set var="bankEnabled"
                                    value="${methodMap['PAYMENT_METHOD_BANK_TRANSFER_ENABLED'] != null && methodMap['PAYMENT_METHOD_BANK_TRANSFER_ENABLED'].policyValue == 'true'}" />
                                <div
                                    class="group relative p-5 bg-surface-container-low rounded-2xl border-2 border-transparent hover:border-primary/20 transition-all ${bankEnabled ? '' : 'opacity-60'}">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center gap-4">
                                            <div
                                                class="w-10 h-10 rounded-lg bg-white flex items-center justify-center shadow-sm text-secondary font-bold">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-[11px] text-on-surface-variant italic">Quét mã QR hoặc
                                                    nhập số TK</p>
                                            </div>
                                        </div>
                                        <label class="relative inline-flex items-center cursor-pointer">
                                            <input type="checkbox" name="PAYMENT_METHOD_BANK_TRANSFER_ENABLED"
                                                value="on" ${bankEnabled ? 'checked' : '' } class="sr-only peer"
                                                onchange="toggleCardOpacity(this)">
                                            <div
                                                class="w-11 h-6 bg-outline-variant peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary">
                                            </div>
                                    ...
                                    <div class="mt-3 pt-3 border-t border-outline-variant/30 text-xs text-primary font-semibold flex items-center gap-1 cursor-pointer hover:underline"
                        ...
                        <div class="bg-primary-container/20 p-6 rounded-3xl border border-primary/10 shadow-sm">
                            <div class="flex gap-4">
                                ...
                                <div>
                                    <p class="text-sm font-bold text-on-primary-container">Bảo mật giao dịch doanh
                                        nghiệp</p>
                                    <p class="text-xs text-on-primary-container/80 mt-1 leading-relaxed">
                                        Mọi quy tắc và cấu hình ngân hàng giao dịch trực tuyến đều tuân thủ chặt chẽ
                                        tiêu chuẩn bảo mật ngân hàng Việt Nam, liên kết trực tiếp với sinh QR thông minh
                                        (VietQR) theo tiêu chuẩn NAPAS 24/7.
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                <div class="w-full">
                    <div
                        class="bg-surface-container-highest/40 rounded-3xl p-8 border border-outline-variant/60 shadow-sm">
                        <div class="flex items-start gap-8">
                            <div
                                class="w-24 h-24 rounded-2xl overflow-hidden shrink-0 border border-outline-variant bg-surface-container-high flex items-center justify-center">
                                <span class="material-symbols-outlined text-outline text-5xl"
                                    style="font-variation-settings: 'FILL' 0">description</span>
                            </div>
                            <div class="flex-1">
                                <h5 class="text-base font-bold text-on-surface mb-2">Tóm tắt điều khoản tài chính cho
                                    khách thuê</h5>
                                <p class="text-xs text-on-surface-variant leading-relaxed">
                                    Hệ thống sẽ tự động hiển thị và cập nhật các "Điều khoản thanh toán & Chính sách
